...
is what exercises the composite `nnx_aux` item, the RNG/dropout persistence, and
the missing-parameter guard together the way training actually hits them.
"""
...
class TestNNXCheckpointRoundTrip(unittest.TestCase):
  ...
  def test_rng_dropout_state_survives_round_trip(self):
    ...
    # rngs/dropout is persisted as a separate on-disk item, not folded into `items`.
    self.assertIn("nnx_aux", os.listdir(os.path.join(self._dir, "ckpt", "1")))
  ...
  def test_missing_aux_dir_falls_back_to_default_rng(self):
    """A checkpoint without nnx_aux (Linen-trained / pre-fix) still loads; rng gets the base default."""
    manager, _ = self._save_step()
    shutil.rmtree(os.path.join(self._dir, "ckpt", "1", "nnx_aux"))
    restored = self._restore(manager, _Model)
